...
`install.sh` is idempotent: it verifies the host is Apple Silicon, installs `mlx-lm` via `pipx`, pre-pulls the default model, and symlinks `mlx-diff` into `~/.local/bin`. Re-run it any time to update.
